...
interfaces. The MicroNet annotation tool allows defining the interface.
MicroNet code assists offer the interface to the developer. MicroNet annotation
processing exports the \gls{api} and automatically integrates the \gls{api} into
the generated service executable file. The API gateway is responsible for
enforcing security policies to regulate access control to restrict parts of the
\gls{api}. It is also responsible to offer the \gls{api} to clients. \mss{} can
access the \gls{api} by only using the networking functionality of MicroNet.
...
cohesion and can be used to define the service \gls{api}.
...
design. \ms{} applications are distributed systems by definition.
...
application management. Its Launch Utility provides shortcuts to compose an
...
In MicroNet this is achieved by using the shared model approach to make message
semantics known throughout the application.
...
de facto standard. I have to admit that I did not evaluate other container
technologies due to the dominance of Docker. I believe Docker will maintain a
strong position because it is being permanently improved.
...
can also be used to contribute a proposal to the practitioner questions.\\
...
tools. Both Amazon Cloud and Google cloud have only been evaluated via
...
\ms{} application behaviour. But for the game domain the traditional RESTful HTTP
approach is just not suited. A more flexible and optimization friendly
communication solution is needed. A message broker perfectly fulfills this role.
...
concepts. The Shared Model and API represent the solution to overcome
...
which is a machine readable language. Machine readability is used for code
generation in order to make the Service API accessible by \mss{}.\\
...
consistency is offered by NoSQL databases like Couchbase. End-to-end data
integrity is only needed for financially relevant actors as mentioned above.
...
an exhaustive answer to this question. \mss{} can indeed talk to each other and
I find this approach very useful to distribute application behaviour over
