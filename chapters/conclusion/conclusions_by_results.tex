...
This conclusion represent my proposal for the most important points that need to
be considered when an \og{} is realized with \mss{}. It summarized all important
aspects of this thesis which are relevant fore \ms{} \og{} development.

\subsection{\ms{} Tenet Guidelines}

I suggest using the \ms{} tenets to design an \og{} application using a \ms{}
driven design. In this section I provide guidelines on how to suffice each
tenet.
...
exposing the API offered by the \mss{}. This allows other  \mss{} to use the
interface in a very specialized way (only accessing the functionality truly
needed). The major challenge in to make the interface available to all services
and clients.
...
CodeAssists offers the interface to the developer. MicroNet annotation
...
influenced by the DDD tenet. Service granularity has been discussed in
\todo{ref thesis 2} in detail.
...
approach to achieve service confinement and to define its interface.
...
the application according to the problem domain (game under development).

\subsubsection{IDEAL}

\paragraph{Isolated State}

To make game services stateless is a task that is up to the developer of the
...
relational or NoSQL database. This is the only state representation of a truely
stateless \og{}. 
...
that is expected to be available quickly. In cases when the processing service
needs longer to process a message than the timeout it must send a notification
to the requesting peer to convey that the result will be sent later
asynchronously. The problem here is that the asynchronous result will never be
sent, maybe because the processing service fails. This can lead to lost
transaction. Persistent logging of requests is the only way to cope with this
problem.
...
Finally the fact that games are state-full in general \todo{cite} has an
influence on state isolation of a \ms{} \og{} application. The simulating
services which are not state-full add additional complexity to the system
because each of these services must be reachable individually. For this purpose
\mn{} uses the game session concept. It defines the process that is necessary to
start, stop, monitor and access game sessions running on simulation services.
\todo{Matchmaking process}

	
\paragraph{Distribution}

The distribution aspect is already tackled by using a \ms{} application design.
\mss{} in general build a distributed system by definition.
...
number can be deployed in parallel to support player demands. 

But there might be cases that a service may only exist once (for example the
round service in the sample game\todo{ref}). These singleton services are not
elastic can cannot scale. These services must be as minimalistic as possible and
it must be able to restart them on failure without loosing the game application
state.

Since the storage layer is extracted from the \mss{} it can introduce a
bottleneck. To cope with this the scaling functionality for the used storage
technology must be used. MicroNet uses a Couchbase NoSQL database which can be
scaled very well maintaining a quorum for data consistency. 

A relational database always represents a bottleneck. But in cases a a
relational database is necessary like for example for sensitive data where ACID
transactions are required it is important to keep these database as slim as
possible. Having multiple relational databases on multiple physical host can
also help to relieve databases. 
...
and Maven both offer CLI which is very well suited for automation with shell
scripts. Git allows to automate the source control and also offers a CLI that
can be automated using scripts. Although not evaluated in this thesis, Jenkins
provides a way to completely automate the build and test process. Jenkins
supports git and maven and is therefore very well suited for MicroNet.

MicroNet also provides a selection of tools to help automate \ms{} application
management. The Launch Utility provides shortcuts compose the application and
...
Loose coupling can be achieved by logically compose the services using a shared
mechanism and leaving the implementation to the service. The physical
composition which might introduce service coupling can be abstracted using a
composition engine which can be seen as a black box to minimize dependencies. .

MicroNet achieves loose coupling by relying on the message broker as the only
physical dependency. As soon as a service can communicate with message broker it
is able to participate within the \ms{} application. 
...
It allows services to store data and not care about what is happening with it.
The format of the data in the session is chosen according to the shared model.
...
foreign technology in a application. It allows to choose the best suited
...
The polyglot programming strategy is used in MicroNet by also supporting C\#
...
the MicroNet Networking functionality is available in the foreign language.
ActiveMQ which is used for MicroNet messaging offers many access possibilities
which allows the integration af many programming languages to be compatible with
MicroNet.

\subsubsection{Lightweight Containers}

Containers are the enabling technology for \ms{} application. Without containers
it would be nearly impossible to maintain the state of a \ms{} application. 
...
technologies because of this reason. I believe Docker will maintain strong
position because the latest improvements of the engine are quite promising.
...
very easily even at run-time performing a rolling update. New functionality can
be added by introducing new services.

One aspect specifically for \og{} is the delivery of the game client containing
all graphical assets to display the game on the client. When service
functionality changes it can be necessary that a new client version needs to
be shipped. The patching is very noticeable to the players.

\subsubsection{DevOps}

During the development of a \ms{} application DevOps tasks are very frequent.
The responsibility to bring and maintain the application to the production
environment is a very excessive process which is not over after application
release. 

Because each service can have multiple dependencies that each must be compatible
with the system the DevOps effort grows with the application. 
...
engineer only must execute instead of additionally writing it. 
