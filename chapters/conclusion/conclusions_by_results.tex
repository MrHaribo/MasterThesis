...
regulate access control to restrict parts of the API. It is also responsible to
offer the API to clients. \mss{} can access the API by only using the
...
influenced by the DDD tenet. A service should only offer the API for one
specific aspect of the application domain. Violation of the single
...
all technical requirements. DDD is also a useful approach to achieve a
high service cohesion and can be used to define the service API.
...
can be scaled very well maintaining a quorum on Json documents for data
consistency.
...
spread the load out to multiple relational databases.
