...
The MicroNet Launch utility locally provides shortcuts for all deployment steps.
...
cloud deployment is therefore to execute the necessary console commands by hand
...
The Shared Model and the Shared \gls{api} are the only concepts needed to
logically compose a \ms{} application. The Shared Model is needed to provide a
common data access layer facilitating session management and a durable
persistence functionality. The Shared \gls{api} concept is needed to define the
format and the semantics of the message transfers.

For physical composition it is enough to rely on a composition engine like
Docker Swarm. Since these engines are very good at abstracting from physical
...
inspired by choreographical composition. Also the loose coupling achieved
through the message broker facilitates choreography.\\
...
\gls{api}. MicroNet then provides assistance via Code Assist in offering the
Shared \gls{api} to developers of other \mss{} who then can access the
\gls{api} via MicroNet messaging. This is done to rely on functionality provided
by other \ms{} teams without knowing any implementation details and to use the
networked \gls{api} functionality like you would use a library via local
function calls.
